/* global Component, DrawingObject, ImageButton, Style, Language, ToggleBar, ChartEvent, Chart */
/**
 * @constructor
 * @extends {Component}
...
function DrawingToolbar(p, id, delegate) {
    ...
    this._params = p;
    if ((this._params["view"] === "you") || (this._params["view"] === "br")) {
    ...
    for (var i = 1; i < DrawingObject.NUM_TOOLS + 1; i++) {
...
DrawingToolbar.prototype.onCustomEvent = function(e) {
    ...
    if (e._source === this._snapButton) {
        ...
        this._chart._selectedColour = this._chart._objectColour;
        this._chart._objectSelected = -1;
        this._chart.resetToolbarColor();
        this._chart.repaint();
        this._chart.process();
...
DrawingToolbar.prototype.setOrientation = function(vertical) {
    if (vertical) {
        var contentHeight = Component.TOOLBAR_BUTTON_SIZE + Component.COMPONENT_X_GAP + Component.TOOLBAR_BUTTON_SIZE * (DrawingObject.NUM_TOOLS + 1);
        this._snapButton.setLocation(0, Math.max((this.getHeight() - contentHeight) / 2, 0));
    } else {
        var contentWidth = Component.TOOLBAR_BUTTON_SIZE + Component.COMPONENT_X_GAP + Component.TOOLBAR_BUTTON_SIZE * (DrawingObject.NUM_TOOLS + 1);
    ...
    var xBase = vertical ? 0 : this._snapButton.getX() + this._snapButton.getWidth() + Component.COMPONENT_X_GAP;
    var yBase = vertical ? this._snapButton.getY() + this._snapButton.getHeight() + Component.COMPONENT_Y_GAP : 0;
...
DrawingToolbar.prototype.setDrawMode = function(mode) {
    mode++;
    for (var i = 0; i < DrawingObject.NUM_TOOLS + 1; i++) {
