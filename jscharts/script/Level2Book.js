/* global Component, OrderBook, Inset, Style, ChartEvent, Strings, DateLabel, IntegerLabel, Label, NumberFormat, List */
/**
 * ----------
 * Level2Book
 * ----------
...
Level2Book.prototype._loadOrders = function() {
    if (!this._shown) 
        return;
    var v = this._showPrices ? this._prices : this._orders;
    var co = undefined; // Order
    var size = undefined; // IntegerLabel
    var price = undefined; // Label
    var time = undefined; // DateLabel
    var code = undefined; // Label
    var j = 0;
    if (this._changes) {
        this._changes = false;
...
Level2Book.prototype._updateBestPrice = function() {
    for (var i = 0; i < this._prices.length; i++) {
        ...
        if (co._price < Level2Book.ZERO_PRICE) 
            continue;
        var hasNonPlus = false;
        for (var j = 0; j < this._orders.length; j++) {
            var co2 = this._orders[j];
            if (co2 === undefined) 
                break;
...
Level2Book.prototype._setupList = function() {
    ...
    for (var i = 0; i < this._numRows; i++) {
        ...
        var d = new DateLabel(this._orderList._id + "_time", this._orderList);
        d.setFormat("*s");
        c[this._timeColumn] = d;
        
        var l = new DateLabel(this._orderList._id + "_code", this._orderList);
        c[this._codeColumn] = l;
        
        var ji = new IntegerLabel(this._orderList._id + "_size", this._orderList);
...
Level2Book.prototype.orderBookLoad = function() {}
/** @static */
...
Level2Book_List.prototype.draw = function(force) {
    ...
    var tid = this._id + "_table";
    var str = "<table id=\"" + tid + "\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" style=\"width:100%;height:100%\" \>";
    var rid;
