/* global Canvas, Chart, Color, Font, Style, Language, Main, LabelIterator */
/**
 * -----------
 * ChartCanvas
 * -----------
...
ChartCanvas.prototype.draw = function(force) {
    ...
    if ((this._chart._features & Chart.F_NO_COPYRIGHT) !== Chart.F_NO_COPYRIGHT) {
        ...
        var copyright = Language.getString("copyright_advfn");
        fontW = this._gc.measureText(copyright).width;
        var fontH = 11;
        this.setFillColor(this._g_border);
    ...
    this.drawXGrid(Color.lightGray, Color.gray, Color.black);
    for (var i = 1; i <= Chart.MAX_STUDIES; i++) {
        if (typeof this._overlays[i] !== "undefined") {
            this._overlays[i].draw(force);
        }
    }
    this._overlays[0].draw(force);
    for (var j = 0; j <= Chart.MAX_STUDIES; j++) {
        if (typeof this._overlays[j] !== "undefined") {
            this._overlays[j].fillBorder();
        }
    }
 ...
 */
ChartCanvas.prototype.drawXGrid = function(grid_col, day_col, tick_col) {
    ...
    var curPos = 0, curWidth = 0;
    var curString;
    var it = new LabelIterator(this._chart, true);
    var mls = [];
    ...
    var idx = 0;
    while(it.valid()) {
        var cmls = idx === mls.length ? undefined : mls[idx];
        if ((this._chart._features & Chart.F_MASK_X_LABELS) !== Chart.F_NO_X_LABELS) {
...
ChartCanvas.prototype.drawXLabels = function() {
    
    var curPos = 0, curWidth = 0;
    var curString;
    var curY = this._chart._drawY + this._chart._drawHeight + this._fontSize + 1;
    var it = new LabelIterator(this._chart, true);
    var mls = [];
    this.setLineWidth(1.0);
    while (it.valid()) {
...
ChartCanvas.prototype._drawDisplayInfo = function() {
    var si = this._chart._currentSymbol.getSymbolInfo(0);
    var infoline = si._displaySymbol;
    if (this._fullInfo) {
        infoline += " (" + si._displayName + ")";
    }
    var fontW = this._gc.measureText(infoline).width;
    this.setLineWidth(1.0);
    this.setFillColor(Color.black);
...
ChartCanvas.prototype._drawDrawingObjects = function(force) {
    ...
    for (var i = 0; i < this._chart._currentSymbol._drawObjects.length; i++) {
        var curObj = this._chart._currentSymbol._drawObjects[i];
        if (curObj) 
            curObj.draw(force);
    }
}
...
ChartCanvas.prototype._drawVerticalLine = function() {
    if (this._chart._currentSymbol._verticalLinePosition < 0) 
        return;
    var myPrice =  this._chart.getSeries(Chart.S_CUR_CLOSE).get(this._chart._mainOverlay.descaleXForMouse(this._chart._currentSymbol._verticalLinePosition));
    var myYI = this._chart._mainOverlay.getY(myPrice);
...
ChartCanvas.prototype._drawCrosshair = function(x, y) {
    ...
    for (var i = 0; i <= Chart.MAX_STUDIES; i++) {
        if (this._overlays[i] === undefined) 
            continue;
        var bottomY = this._overlays[i]._y + this._overlays[i]._height;
        if (y < bottomY - 1) {
            myOverlay = i;
            break;
        }
    ...
    var myPrice = this._overlays[myOverlay].descaleY(y);
