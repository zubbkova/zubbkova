...
function ChartSymbolSet(l, c, s, f, p) {
    ...
    if (arguments.length === 3) {
        ...
        let pp = parseInt(s.substring(0, s.indexOf(',')), 10);
        this.init(c, pp);
    } else if (p) {
...
ChartSymbolSet.prototype.initialise = function(newSyms) {
    ...
    if (!this._isValid)
        return;
    let inf = SymbolSet.prototype.getSymbolInfo.call(this, 0); // ChartInfo
    ...
    let dd = new Date(this._parent._timestamp.getTime() - this._delay * 60000);
...
ChartSymbolSet.prototype.initialiseStockStudies = function(style) {
    let stock = StudyStock.createMainStock(this._chart._mainOverlay);
    this._chart._mainOverlay._study[Overlay.MAIN_STUDY_ID] = stock;
    stock._style = style;
    let numOverlays = this._entries.length - 1;
    for (let i = 0; i < Chart.MAX_OVERLAYS; i++) {
        let newSym = '';
        let overlayStyle = StudyStock.ST_NORMAL;
        if (i < numOverlays) {
            if (!this._chart._parent.getNoToolbar()) {
                newSym = this.getDisplaySymbol(i + 1);
                for (let j = 0; j < this._chart._toolbar._overlaySymbols.length; j++) {
                    let se = this._chart._toolbar._overlaySymbols[j]; // SymbolEdit
                    if (se.getSymbol() === undefined) continue;
                    if (se.getSymbol() === newSym) {
                        for (let m = 0; m < this._chart._toolbar._overlayStyleButtons[j].length; m++) {
...
//            console.log("new Overlay", Overlay.OVERLAY_STUDY_ID + i);
            if (this._chart._mainOverlay._study[Overlay.OVERLAY_STUDY_ID + i] === undefined || this._chart._mainOverlay._study[Overlay.OVERLAY_STUDY_ID + i]._name !== this._chart._mainOverlay._chartCanvas._chart._currentSymbol.getDisplaySymbol(i + 1)) {
...
ChartSymbolSet.prototype.initialiseFormatter = function(pricesToCheck, maxCharacters) {
    ...
    let market = this.mainSymbol().substring(0, this.mainSymbol().indexOf('^'));
    if (market === 'FX') {
        ...
        this._yLabelSuffix = 0;
        maxCharacters--;
        let sfs = this._calcDPAndWidthValues(pricesToCheck); // Array
        ...
        let dpAllowed = maxCharacters - sfs[ChartSymbolSet.MAX_PLACES_BEFORE_DP];
        this._labelFormatter.setMinimumFractionDigits(0);
...
ChartSymbolSet.prototype.initialiseDateRangePeriod = function(from, to) {
    if (!this._isValid)
        return;
    let masterTimeList = this.getMasterTimeList();
...
ChartSymbolSet.prototype.initialiseRangeFromPeriod = function(resetRange) {
    ...
    if (resetRange || this._timeEnd === undefined) {
        ...
        if (this.mainFrequency() !== PriceDataConstants.FREQUENCY_D) {
            ...
            let myCal = Calendar.getInstance();
            myCal.setTime(this._time);
            let day = myCal.get(Calendar.DAY_OF_WEEK);
        ...
        let newTimeEnd = masterTimeList.convertTime(this._timeEnd, this.mainFrequency());
    ...
    let maxRange = this._chart.calcMaxStudyRange();
...
ChartSymbolSet.prototype.getYLabel = function(val) {
    ...
    let l = this._labelFormatter.format(val / this._yLabelFactor) + ChartSymbolSet.SI_SUFFIXES[this._yLabelSuffix];
    if (this._showPercentageChanges) {
        l += "%";
        if (val > 0.0) l = "+" + l;
    }
...
ChartSymbolSet.prototype.getDescriptionText = function() {
    let t = this._displayName + ' ' + Language.getString(this.mainFrequency() <= PriceDataConstants.FREQUENCY_60 ? 'toolbar_intraday' : 'toolbar_historical');
    return t;
}
/**
...
ChartSymbolSet.prototype.calculateScaleFactors = function() {
    for (let j = Chart.S_BID_CLOSE; j <= Chart.S_CUR_CLOSE; j++) {
        let series = SymbolSet.prototype.getSeries.call(this, 0, j);
        if (series) 
    ...
    for (let i = 1; i < this._entries.length; i++) {
        for (let j = Chart.S_BID_CLOSE; j <= Chart.S_CUR_CLOSE; j++) {
            let series = SymbolSet.prototype.getSeries.call(this, i, j);
            if (series) 
                series.setScaleFactor(1.0);
        }
    }
    // Diana: this makes scales for price
//    let series = this.getSeries(0, Chart.S_CUR_CLOSE);
//    let initialStart = this._timeStart;
//    if (series) {
//        initialStart = Math.max(series.timeStart(), this._timeStart);
//    }
//    for (let i = 1; i < this._entries.length; i++) {
//        let myI = TimeIterator.forwardRangeIterator(this.getMasterTimeList(), initialStart, this._timeEnd);
//        let overlayPrice = undefined;
//        let overlayClose = this.getSeries(i, Chart.S_CUR_CLOSE);
//        do {
//            overlayPrice = overlayClose.getUnscaled(myI._d);
//        } while (overlayPrice === undefined && myI.move());
//        let mainPrice = this.getSeries(0, Chart.S_CUR_CLOSE).getUnscaled(myI._d);
//        for (let j = Chart.S_BID_CLOSE; j <= Chart.S_CUR_CLOSE; j++) {
...
ChartSymbolSet.prototype.getSymbolInfo = function(index) {
    let si = SymbolSet.prototype.getSymbolInfo.call(this, index);
    if (this._displayCurrency && !si.isCurrency(this._displayCurrency)) {
        let sym = "FX^" + si.getForexCurrency() + this._displayCurrency;
        let forexData = this._forexSymbolSet.getData(sym, PriceDataConstants.FREQUENCY_D);
    ...
    if (this._relativeSymbol) {
        let relativeData = this._relativeSymbolSet.getData(this._relativeSymbol, this.mainFrequency());
...
ChartSymbolSet.prototype.setRelativeDisplay = function(symbol, override) {
    ...
    if (symbol === undefined) {
        ...
        let currentKeys = this._relativeSymbolSet ? [this._relativeSymbolSet.allEntryKeys()] : [];
        let newSymbol = !currentKeys.includes(symbol + this.mainFrequency());
        let syms = newSymbol ? [symbol] : [];
        if (syms.length === 0) {
            this.loadCompleted();
        } else {
...
ChartSymbolSet.prototype.setShowChange = function(show, override) {
    ...
    if (this._showPercentageChanges) {
        for (let i = 0; i < this._entries.length; i++) {
            for (let j = 0; j < DataAggregator.NUM_SERIES; j++) {
...
ChartSymbolSet.prototype.loadCompleted = function() {
    if (this._displayCurrency) {
        for (let i = 0; i < this._entries.length; i++) {
            let curSymInfo = this._parent._symbolInfo.get(this._entries[i]._symbol);
            ...
            let sym = "FX^" + curSymInfo.getForexCurrency() + this._displayCurrency;
            let forexData = this._forexSymbolSet.getData(sym, PriceDataConstants.FREQUENCY_D);
            for (let j = 0; j < DataAggregator.NUM_SERIES; j++) {
...
ChartSymbolSet.prototype.setDisplayCurrency = function(c, override) {
    ...
    if (c === undefined) {
        ...
        let currentForexSyms = new Set();
        if (this._forexSymbolSet) {
            currentForexSyms.add(this._forexSymbolSet.symbols());
        }
        let newForexSyms = new Set();
        for (let item of this._entries) {
            let curSymInfo = this._parent._symbolInfo.get(item._symbol);
            if (!curSymInfo.isCurrency(this._displayCurrency)) {
                let sym = "FX^" + curSymInfo.getForexCurrency() + this._displayCurrency;
                newForexSyms.add(sym);
            }
            // Only get data for items not already in the FOREX set.
            let addedForexSyms = newForexSyms.difference(currentForexSyms);
            let syms = addedForexSyms.toArray();
            if (syms.length > 0) {
                // Load symbol info first.
                this._parent.getChartInfo(syms);
...
ChartSymbolSet.prototype.getDrawingObjects = function() {
    let objects = new Map();
    for (let i = 0; i < this._drawObjects.length; i++) {
        objects.set('do' + i, this._drawObjects[i].getStringForm());
    }
    return objects;
}
/**
...
ChartSymbolSet.prototype.getLabelScale = function() {
    ...
    if (this._chart._currentSymbol === undefined) 
        return -1;
    let labelScale = this._currentLabelScale;
    let displayedTime = this._timeEnd.getTime() - this._timeStart.getTime();
...
ChartSymbolSet.prototype.recalcXScale = function(dx) {
    ...
    this._numTimeUnits = Math.trunc(this._chart.getDrawGraphWidth() / this._unitWidth);
...
ChartSymbolSet.prototype.setTimeEnd = function(d) {
    ...
    let mtl = this.getMasterTimeList();
    if (mtl) {
...
ChartSymbolSet.prototype.changeFrequency = function(p, f, reset) {
    if (f !== this.mainFrequency()) {
        let newFreqList = new Array(this._entries.length);
        newFreqList.fill(f);
...
ChartSymbolSet.prototype._calcDPAndWidthValues = function(values) {
    ...
    for (let i = 0; i < values.length; i++) {
        let curValue = Math.abs(values[0]);
        if (curValue === 0.0) 
            continue;
        let floorValue = Math.floor(curValue);
        let digitsBefore = 1 + Math.floor(Math.log(floorValue) / Math.log(10.0));
        let totalDigits = digitsBefore;
        if (floorValue !== curValue) {
            let fracValue = curValue - floorValue;
            let decimalPlaces = 0;
            while (fracValue < 1.0) {
                decimalPlaces++;
                fracValue *= 10.0;
...
ChartSymbolSet.prototype._removeFromAllProxies = function(k) {
    for (let i = 0; i < this._entries.length; i++) {
        for (let j = 0; j < DataAggregator.NUM_SERIES; j++) {
            if (this._entries[i]._series[j].constructor.name === "RelativeSeries")
