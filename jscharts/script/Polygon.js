...
Polygon.prototype.calculateBounds = function(xps, yps, nps) {
    let boundsMinX = Number.MAX_SAFE_INTEGER;
    let boundsMinY = Number.MAX_SAFE_INTEGER;
    let boundsMaxX = Number.MIN_SAFE_INTEGER;
    let boundsMaxY = Number.MIN_SAFE_INTEGER;
    for (let i = 0; i < nps; i++) {
        let x = xps[i];
...
Polygon.prototype.inside = function(x, y) {
    ...
    let hits = 0;
    let lastx = this._xpoints[this._npoints - 1];
    let lasty = this._ypoints[this._npoints - 1];
    let curx, cury;

    // Walk the edges of the polygon
    for (let i = 0; i < this._npoints; lastx = curx, lasty = cury, i++) {
