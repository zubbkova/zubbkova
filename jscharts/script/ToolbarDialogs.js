/* global StudyFactory, StudyDialog, Component, Color, Label, Language, Button, ChartEvent, EditBox */
/**
 * --------------
 * ToolbarDialogs
 * --------------
 */
var ToolbarDialogs = new Object();
/**
 * @param {string} id
...
ToolbarDialogs.getStudyDialog = function(id, delegate, study) {
    var c = StudyFactory.studies[study.getMnemonic()];
    var items = c.getItems(); // StudyDialog_StudyParameter
    if (!items || items.length === 0) {
        return new ToolbarDialogs_StudyNoParametersDialog(id, delegate);
    }
    var d = new StudyDialog(id, delegate);
    for (var i = 0; i < items.length; i++) {
        d.addParameter(items[i]);
    }
    d.init2(study);
    return d;
}
...
function ToolbarDialogs_StudyNoParametersDialog(id, delegate) {
    ...
    this.setSize(150, 60);
    this._paramItems = new Object();
    this._currentY = 8;
    
    var l1 = new Label(this._id + "_l1", this);
    ...
    this._cancelButton.setBounds((this._width - 60) / 2, this._currentY, 60, 20);
    this.add(this._cancelButton);
    this.setSize(150, this._cancelButton._y + this._cancelButton._height + 8);
}
/**
 * Inheriting
 */
...
/** @override */
ToolbarDialogs_StudyNoParametersDialog.prototype.onCustomEvent = function(e) {
    if (e._event === ChartEvent.BUTTON_CLICK) {
        if (e._source === this._cancelButton) {
