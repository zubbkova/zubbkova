/* global Component, Color, Utils, Button, ChartEvent, Label, EditBox, RadioButtonGroup, Checkbox */
/**
 * -----------
 * StudyDialog
 * -----------
...
StudyDialog.prototype.init2 = function(s) {
    this._target = s;
    var items = Utils.convertStringParams(s.getParams(), ":", "-");
    var self = this;
    var prop;
    for (prop in items) {
        var p = self._paramItems[prop];
        p.attach(s);
        p.set(items[prop]);
    }
    var size = 0;
    for (prop in this._paramItems) {
        size++;
    }
    this.addOkCancelButtons(this._currentY, size > 0, true);
}
/**
 * @param {number} yLoc
...
StudyDialog.prototype.getParams = function() {
    var bits = [];
    for (var prop in this._paramItems) {
        bits.push(prop + "-" + this._paramItems[prop].get());
    }
    return bits.join(":");
}
/** @override */
...
StudyDialog_StudyEditParameter.prototype.create = function(d) {
    var paramLabel = new Label(d._id + "_label" + this._name, d);
...
StudyDialog_StudyButton.prototype.set = function() {}
/**
 * @param {StudyDialog} d
 */
StudyDialog_StudyButton.prototype.create = function(d) {
    var width = this._label.length * 8;
