...
Component.prototype.process = function(t, res) {
    for (let i = this._child.length - 1; i >= 0; i--) {
 ...
 */
Component.prototype.feedDelegate_loadingComplete = function(contents) {}
/**
...
Component.prototype.setStyle = function(slot, style) {
    ...
    let fontSize = Style.getFontSize(Style.DEFAULT);
    let fontStyle = Style.getFontStyle(Style.DEFAULT);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (fontSize !== -1 && fontStyle !== -1) {
            let font = Style.getFont(fontSize, fontStyle);
...
Component.prototype.getAbsoluteBounds = function() {
    let p = this.getAbsoluteLocation();
