/* global Component, Utils, Main, ErrorCodes, WebQuery_Table, Dimension, Calendar, Chart, ChartEvent, ToolbarDialogs_PrefsDialog, Language, Label, ToolbarDialogs, Study, ChartSymbolSet, Currency, PriceDataConstants, ComboBox, Color, Button, UIImage, Style, Overlay, StudyFactory, Tabs, Tabs_data, Checkbox, SymbolEdit, MiscToolbar, ToggleBar */
/**
 * -------
 * Toolbar
 * -------
...
Toolbar.prototype.loadMemos = function(symbols) {
    if (typeof symbols === "string") {
        var memo = Utils.replace(symbols, "%%", "\n");
    ...
    var memos = new Array(1);
    if (Toolbar.memos_loaded === undefined) {
        Toolbar.memos_loaded = new Object();
    }
    if (Toolbar.memos_loaded.hasOwnProperty(symbol) && Toolbar.memos_loaded[symbol] !== undefined) {
        memos[0] = Toolbar.memos_loaded[symbol].toString();
        ...
        var params = encodeURIComponent("symbols=" + tmp);
        var self = this;
        $.ajax({
            type: "POST",
...
Toolbar.prototype._parseMemoResponse = function(response, symbols) {
    ...
    var error = parseInt(responseArray[0], 10);
    if (error !== 0) {
        console.log("Toolbar. Can't load memo: " + ErrorCodes.strings[error]);
        return;
    }
    var numRows = parseInt(responseArray[1], 10);
    var numColumns = parseInt(responseArray[2], 10);
    ...
    if (table._contents[0][0]) {
        ...
        for (i = 0; i < symbolsArray.length; i++) {
            Toolbar.memos_loaded[symbolsArray[i]] = (memos.length > i) ? memos[i] : "";
        }
    }
    this.updateMemos(memos);
}
...
Toolbar.prototype.setBounds = function(x, y, w, h) {
    ...
    for (var i = 0; i < Toolbar.NUM_OVERLAYS; i++) {
        var k = 4 + i * 24;
        this._overlayStyleButtonContainer[i].setBounds(this._overlayTab.getWidth() - Component.COMPONENT_X_GAP - (this._overlayStyleButtons[i].length * Component.TOOLBAR_BUTTON_SIZE), k, this._overlayStyleButtons[i].length * Component.TOOLBAR_BUTTON_SIZE, 24);
        this._overlaySymbols[i].setBounds(2, k, this._overlayStyleButtonContainer[i].getX() - Component.COMPONENT_X_GAP, 24);
...
Toolbar.prototype.getChartSize = function() {
    ...
    var w = Toolbar.sizes[this._sizeCombo.getIndex()][0];
    var h = Toolbar.sizes[this._sizeCombo.getIndex()][1];
...
Toolbar.prototype.getDate = function(i) {
    var day = this._dateRange[i][Toolbar.DR_DAY].getIndex() + 1;
    var month = this._dateRange[i][Toolbar.DR_MONTH].getIndex() + 1;
    var year = this._dateRange[i][Toolbar.DR_YEAR].getIndex() + 1970;
...
Toolbar.prototype.setOverlayStyle = function(overlay, style) {
    ...
    if (this._chart && this._chart._canvas._overlays && this._chart._canvas._overlays[0]) {
        var overlayOffset = Chart.MAX_STUDIES + 3;
...
Toolbar.prototype._setSymbolComponentsFromSymbolSet = function(ss) {
    ...
    if (!ss) {
        ...
        for (i = 0; i < this._overlaySymbols.length; i++) {
            this._overlaySymbols[i].setSymbol('');
        }
    } else {
        this._mainSymbol.setSymbol(ss.mainSymbol());
        for (i = 1; i < ss._entries.length; i++) {
            this._overlaySymbols[i - 1].setSymbol(ss._entries[i]._symbol);
        }
        for (i = Math.max(ss._entries.length - 1, 0); i < this._overlaySymbols.length; i++) {
            this._overlaySymbols[i].setSymbol("");
        }
    }
}
    ...
    switch (e._event) {
        ...
        case ChartEvent.BUTTON_CLICK:
            if (e._source === this._ok) {
                ...
                var scheme = this._schemeCombo.getValue();
                if (scheme !== "Default") {
                ...
                for (i = 0; i < Toolbar.NUM_STUDIES; i++) {
                    if (e._source === this._studyEditButtons[i]) {
                        if (this._chart.getStudy(i)) {
                            ...
                            var r = this._studyEditButtons[i].getAbsoluteBounds();
                ...
                for (i = 0; i < Toolbar.NUM_OVERLAYS; i++) {
        ...
        case ChartEvent.COMBO_SELECT:
            if (e._source === this._periodCombo) {
                ...
                if (!this._baseCurrencyCheckbox.getChecked()) {
                    var curCode = Currency.currencies[this._displayCurrencyCombo.getIndex()];
                ...
                for (i = 0; i < Toolbar.NUM_STUDIES; i++) {
                    if (e._source === this._studyListsCombo[i]) {
                         if (this._studyListsCombo[i].getIndex() !== -1) {
                             var mnemonic = Study.comboMnemonics[this._studyListsCombo[i].getIndex()][1];
                             var help = this._getHelpAnchor(mnemonic);
...
//                this._chart.setOutsideData(this._outsideDataCheckbox.getChecked());
                ...
                if (this._baseCurrencyCheckbox.getChecked()) {
                    ...
                    var currency = Currency.currencies[this._displayCurrencyCombo.getIndex()];
                    this._parent._currentSymbol.setDisplayCurrency(currency, true);
                }
...
Toolbar.prototype.ToolbarDialogs_PrefsDialogDelegate_OK = function() {
    var scheme = this._schemeModal._nameEdit.getText();
    var newScheme = !this._parent._config.hasScheme(scheme);
    if (newScheme) {
...
Toolbar.prototype.handleSymbolSelect = function() {
    var symbol_array = new Array(1 + this._overlaySymbols.length);
    if (this._mainSymbol) {
    ...
    if (this._overlaySymbols) {
        for (var i = 1; i <= this._overlaySymbols.length; i++) {
...
Toolbar.prototype.updateSymbols = function() {
    ...
    if (this._overlaySymbols) {
        for (var i = 0; i < this._overlaySymbols.length; i++) {
            this._overlaySymbols[i].updateSymbol();
        }
    }
...
Toolbar.prototype._initMain = function() {
    ...
    var p = (this._params.hasOwnProperty("period") && this._params["period"] !== undefined) ? parseInt(this._params["period"].toString(), 10) : 0;
    ...
    if (this._params.hasOwnProperty("floatBars") && this._params["floatBars"] !== undefined) {
...
Toolbar.prototype._mainDateRange = function(visible) {
    ...
    var curYear = c.get(Calendar.YEAR);
    var l, day, d, month, m, year, yr;
    for (var i = 1; i >= 0; i--) {
        var y = 5 + i * 24;
        this._dateRange[i] = new Array(3);
        if (Language.getLanguageID() === "jp") {
            year = new ComboBox(this._dateRangeBox._id + (i === 0 ? "_fromYearCombo" : "_toYearCombo"), this);
            year.setBounds(50, y, 60, 20);
            for (yr = 1970; yr <= curYear; yr++) {
                year.addItem(yr.toString());
            }
            this._dateRange[i][Toolbar.DR_YEAR] = year;
            this._dateRangeBox.add(year);
            var slash1 = new Label(this._dateRangeBox._id + (i === 0 ? "_fromSlash1Label" : "_toSlash1Label"), this);
            slash1.setText("/");
            ...
            month = new ComboBox(this._dateRangeBox._id + (i === 0 ? "_fromMonthCombo" : "_toMonthCombo"), this);
            ...
            var slash2 = new Label(this._dateRangeBox._id + (i === 0 ? "_fromSlash2Label" : "_toSlash2Label"), this);
            slash2.setText("/");
            ...
            day = new ComboBox(this._dateRangeBox._id + (i === 0 ? "_fromDayCombo" : "_toDayCombo"), this);
            ...
            l = new Label(this._dateRangeBox._id + (i === 0 ? "_fromLabel" : "_toLabel"), this);
    ...
    if (this._params.hasOwnProperty("from") && this._params["from"] !== undefined) {
        this._setDate(Toolbar.DR_FROM, new Date(60000 * parseFloat(this._params["from"].toString())));
    } else {
    ...
    if (this._params.hasOwnProperty("to") && this._params["to"] !== undefined) {
        this._setDate(Toolbar.DR_TO, new Date(60000 * parseFloat(this._params["to"].toString())));
    } else {
...
Toolbar.prototype._initTabStudies = function() {
    var last = this._tabs.size();
    this._tabs.addTab(this._tabs._id + "_t" + last, Language.getString("tab_studies"));
    this._studyTab = new Tabs_data(this._tabs.getTab(last)._id + "_data", this);
    this._tabs.setDataControl(last, this._studyTab);
    for (var i = Toolbar.NUM_STUDIES - 1; i >= 0; i--) {
        var combo = new ComboBox(this._studyTab._id + "_combo" + i, this);
        var y = 4 + i * 24;
        combo.setBounds(Component.COMPONENT_X_GAP, y, 250, 20);
        Study.comboMnemonics = Study.getMnemonics();
        for (var j = 0; j < Study.comboMnemonics.length; j++) {
        ...
        var e = new Button(this._studyTab._id + "_editButton" + i, this);
        ...
        var sel = "";
        var idx = 0;
        if (this._params.hasOwnProperty("s" + i) && this._params["s" + i] !== undefined) {
            sel = this._params["s" + i];
            idx = Study.studyMnemonicToIndex(sel);
        }
        combo.setIndex(idx);
        var h = new Button(this._studyTab._id + "_helpButton" + i, this);
...
Toolbar.prototype._initTabOverlays = function() {
    var last = this._tabs.size();
    this._tabs.addTab(this._tabs._id + "_t" + last, Language.getString("tab_overlays"));
    this._overlayTab = new Tabs_data(this._tabs.getTab(last)._id + "_data", this);
    this._tabs.setDataControl(last, this._overlayTab);
    this._overlayLoadingLabels = new Array(Toolbar.NUM_OVERLAYS);
    var i;
    for (i = Toolbar.NUM_OVERLAYS - 1; i >= 0; i--) {
        var sc = new SymbolEdit(this._overlayTab._id + "_edit" + i, this);
        ...
        for (var j = 0; j < this._overlayStyleButtons[i].length; j++) {
    ...
    for (i = 0; i < Toolbar.NUM_OVERLAYS; i++) {
        var nexti = (i === Toolbar.NUM_OVERLAYS - 1) ? 0 : i + 1;
...
Toolbar.prototype._initTabCurrency = function() {
    var last = this._tabs.size();
    this._tabs.addTab(this._tabs._id + "_t" + last, Language.getString("tab_currency"));
    this._currencyTab = new Tabs_data(this._tabs.getTab(last)._id + "_data", this);
    this._tabs.setDataControl(last, this._currencyTab);
    ...
    var displayCurrencyLabel = new Label(this._overlayTab._id + "_label", this);
...
Toolbar.prototype._initTabOptions = function() {
    var last = this._tabs.size();
    this._tabs.addTab(this._tabs._id + "_t" + last, Language.getString("tab_options"));
    this._optionTab = new Tabs_data(this._tabs.getTab(last)._id + "_data", this);
    this._tabs.setDataControl(last, this._optionTab);
    ...
    var ls = (this._params.hasOwnProperty("logscale") && this._params["logscale"] !== undefined) ? (this._params["logscale"] === "true") : false;
    ...
    var sizeLabel = new Label(this._optionTab._id + "_sizeLabel", this);
    ...
    var which = 0;
    var cw = parseInt(this._params["cw"].toString(), 10);
    var ch = parseInt(this._params["ch"].toString(), 10);
    for (var i = 0; i < Toolbar.sizes.length; i++) {
    ...
    if ((this._params.hasOwnProperty("floatBars") && this._params["floatBars"] !== undefined) || (this._params.hasOwnProperty("noscale") && this._params["noscale"] !== undefined)) {
        sizeLabel.hide();
...
Toolbar.prototype._initTabPrefs = function() {
    ...
    var schemeLabel = new Label(this._optionTab._id + "_schemeLabel", this);
...
Toolbar.prototype._getHelpAnchor = function(mnemonic) {
    ...
    if (this._parent.getView() === "jp" || this._parent.getView() === "excite") {
        if (mnemonic !== '') {
            hid = StudyFactory.getHelpCode(mnemonic);
            if (hid <= 0) hid = 414;
...
Toolbar.prototype._editMemo = function() {
    var p = new Object();
    p["pid"] = "m_memo";
    var symbol = encodeURIComponent(this._chart._currentSymbol.symbols()[0]);
    p["symbol"] = symbol;
    p["showBack"] = "false";
    p["ref"] = "charts";
    var pp = Utils.convertHashParamsWithoutDefaults(p, "&", "=");
...
Toolbar.prototype.initOverlayStyleButtons = function(ss) {
    if (!ss)
        return;
    var which = 0;
    for (var i = 0; i < Toolbar.NUM_OVERLAYS; i++, which++) {
        var s = this._overlaySymbols[i].getSymbol();
        if (s) {
...
Toolbar.prototype.initialiseState = function(p, transparent) {
    ...
    if (this.getPeriod() === ChartSymbolSet.PERIOD_RANGE && !transparent) {
        if (this._params["from"] !== undefined && this._params["to"] !== undefined) {
            this._setDate(Toolbar.DR_FROM, new Date(parseFloat(this._params["from"].toString()) * 60000));
            this._setDate(Toolbar.DR_TO, new Date(parseFloat(this._params["to"].toString()) * 60000));
        }
    }
    if (this._params.hasOwnProperty("log") && this._params["log"] !== undefined) {
        this._logScaleCheckbox.setChecked(this._params["log"].toString() === "1");
    } else if (!transparent) {
        this._logScaleCheckbox.setChecked(false);
    }
    this._chart.setLogScale(this._logScaleCheckbox.getChecked());
    for (var slot = 0; slot < Toolbar.NUM_STUDIES; slot++) {
        if (this._params.hasOwnProperty("s" + slot) && this._params["s" + slot] !== undefined) {
            var mnemonic = this._params["s" + slot];
            ...
            if (this._params.hasOwnProperty("sp" + slot) && this._params["sp" + slot] !== undefined) {
                if (this._chart) {
                    var p1 = this._params["sp" + slot].toString();
                    this._chart.setStudyParams(slot, p1);
                }
            }
        } else if (!transparent) {
    ...
    if (this._schemeCombo.size() === 1) {
        var schemes = this._parent.getConfig().getSchemes();
    ...
    if (this._params.hasOwnProperty("cw") && this._params["cw"] !== undefined && !transparent) {
        var cw = parseInt(this._params["cw"].toString(), 10);
        var ch = parseInt(this._params["ch"].toString(), 10);
...
Toolbar.DR_YEAR = 2;
/** @static */
Toolbar.sizes = [[648, 400], [648, 600], [648, 800], [648, 1200], [700, 460], [972, 400], [972, 600], [972, 800], [972, 1200], [1296, 400], [1296, 600], [1296, 800], [1296, 1200], [324, 200], [486, 300]];