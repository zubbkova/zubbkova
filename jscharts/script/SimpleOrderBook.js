...
/* global OrderBook, FeedRequest, Feed, FeedContent */
/**
 * ---------------
 * SimpleOrderBook
 * ---------------
...
function SimpleOrderBook(id, params, feedSymbol) {
    ...
    this._line = [];
    this._orders = [];
    var fr = new FeedRequest("orderbook");
...
SimpleOrderBook.prototype.feedDelegate_feed = function(fc) {
    if (this._state === OrderBook.DISCONNECTED) {
        this._orders[OrderBook.BUY] = new Object();
        this._orders[OrderBook.SELL] = new Object();
    ...
    if (fc._id >= 10) {
        ...
        var side = (fc._id === 4) ? OrderBook.BUY : OrderBook.SELL;
        var num = parseInt(fc._contents.substring(1), 10);
...
SimpleOrderBook.prototype.process = function(t) {
    return this._feed && this._feed.process();
}
...
SimpleOrderBook.prototype._doFlow = function(fc) {
    fc._id -= 10;
    var side = fc._id % 2;
    var change = OrderBook.NO_CHANGE;
    var action = parseInt(fc._id / 12, 10);
    var period = parseInt((fc._id % 12) / 2, 10);
    ...
    var value = FeedContent.getInteger(fc._contents);
    this.sendOrderFlow(side, action, period, value);
}
/**
 * @private
 * @param {number} side
...
SimpleOrderBook.prototype._orderAdd = function(side, id, time, code, size, price) {
    var o = new SimpleOrderBook_Order();
    o._id = id.toString();
...
SimpleOrderBook.prototype._orderDelete = function(side, id, time, code, size, price) {
    var oid = id.toString();
    var o = this._findOrder(side, oid);
    if (o) {
        delete this._orders[side][o._id];
...
SimpleOrderBook.prototype._orderUpdate = function(side, id, time, code, size, price) {
    ...
    if (size && size._contents) {
        var nsize = FeedContent.getInteger(size._contents);
        if (nsize < o._size) {
    ...
    if (price && price._contents) {
        var nprice = parseFloat(price._contents);
        if (nprice < o._price) {
            o._lastChange |= OrderBook.PRICE_DOWN;
            o._price = nprice;
