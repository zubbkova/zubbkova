...
MetaStudy.EMA = function(c, source, p, o) {
    ...
    let start = source.timeStart();
    let end = c._parent._currentSymbol._time;
    let smooth, total = 0.0, current = 0.0;
    let n = 0;
    dest.clear();
    let i = TimeIterator.forwardRangeIterator(c.getMasterTimeList(), start, end);
...
MetaStudy.ROC = function(c, source, p) {
    ...
    do {
        if (source.get(c.timeAdjust(i._d, -period)) !== 0.0) {
            let previous = source.get(c.timeAdjust(i._d, -period));
...
MetaStudy.RSI = function(c, source, p) {
    ...
    do {
        let diff = source.get(i._d) - source.get(c.timeAdjust(i._d, -1));
        if (diff > 0.0) {
...
MetaStudy.HighestInArray = function(array) {
    let dest = Number.MIN_SAFE_INTEGER;
...
MetaStudy.LowestInArray = function(array) {
    let dest = Number.MAX_SAFE_INTEGER;
    for (let i = 0; i < array.length; i++) {
...
MetaStudy.sumValuesInSeries = function(c, s) {
    let out = 0;
    let start = c._parent._currentSymbol._timeStart;
    let end = c._parent._currentSymbol._time;
...
MetaStudy.linearRegression = function(c, source) {
    ...
    let a = 0, b = 0;
    if (count !== 0) {
        let sum_xx = MetaStudy.sumSquareValuesInSeries(c, x);
        let sum_xy = MetaStudy.sumValuesInSeries(c, MetaStudy.multiplySeries(c, x, source));
        let Sxy = sum_xy - (mean_x * mean_y * count);
        let Sxx = sum_xx-((mean_x * mean_x) * count);
        b = Sxy / Sxx;
        a = mean_y - (b * mean_x);
    }
