...
Legend.prototype.draw = function() {
    ...
    if (!DrawComponent.prototype.draw.call(this))
        return;
    let cury = this._y + this._inset._top;
    for (let i = 0; i < this._legends.length; i++) {
        let curx = this._x + 2;
        let curLegend = this._legends[i];
        ...
        if (curLegend._bold) {
            let font = this._chartCanvas.getFont();
...
Legend.prototype.resetLocation = function(force) {
    if (this._x === 0 || this._y === 0 || force)
        this.setLocation(this._chartCanvas._chart._drawX + 3, this._parent._y + 3);
}
    ...
    if (!this._shown) {
        ...
        if (this._legendsVisible === 0) {
            ...
            for (let item of this._legends) {
                let textWidth = this._chartCanvas._gc.measureText(item._name).width;
...
Legend.prototype.addItem = function(name, c, b) {
    ...
    let newLegend = new Legend_LegendItem(name, colour);
...
Legend.getNextColour = function() {
    let c = Legend.legendDefault[Legend.staticCurColour++];
