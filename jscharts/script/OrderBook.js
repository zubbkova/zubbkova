...
OrderBook.prototype.addListener = function(listener, side) {
    let l = new OrderBook_Listener(listener);
...
OrderBook.prototype.sendOrderConnect = function() {
    for (let item of this._listeners) {
        item._listener.orderBookConnect(this._id);
    }
}
/**
 * @param {number} side
 ...
 * @param {number} change
 */
OrderBook.prototype.sendOrderFlow = function(side, action, period, value, change) {
    for (let item of this._listeners) {
        if (item._side[side]) {
            item._listener.orderFlow(this._id, side, action, period, value, change);
        }
    }
}
...
OrderBook.prototype.sendOrderLoad = function() {
    for (let item of this._listeners) {
        item._listener.orderBookLoad(this._id);
    }
}
