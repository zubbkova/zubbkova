...
IPC.prototype.run = function() {
    let lastClientProcess = new Date().getTime();
...
IPC.prototype.processClients = function() {
    for (let i = 0; i < IPC.clients.length; i++) {
...
IPC.prototype.processInputBuffer = function(data) {
    ...
    let b = (newDate - this._lastTime) > 500;
    if (b) {
...
//        console.log("IPC. received data: " + data);
    ...
    switch (c0) {
        case '+':
             ...
             */
            let error = parseInt(data.substring(1, data.indexOf(":")), 10);
            console.error(error);
...
IPC.prototype.postConnectClients = function() {
    ...
    IPC.clients.forEach(function(client, i, array){
        client._feed.handleConnect();
    });
}
...
//    console.log("clients length", this.clients.length);
    var client;
    IPC.clients.forEach(function(c, i, array){
...
IPC.clientLogin = function(client) {
    ...
    let str = JSON.stringify({'type': 'subscribe', 'client_id': client._id, 'user': IPC.userName, 'sid': IPC.sid, 'page_key': Main.getParams().get("page_key"), 'app': client._feed._name, 'ipc_tag': IPC.ipcTag, 'request': client._feed._dataBlock});
...
IPC.clientLogout = function(client) {
    ...
    let str = JSON.stringify({type: 'unsubscribe', client_id: client._id, ipc_tag: IPC.ipcTag});
    if (IPC.master._socket.readyState === 1) {
        IPC.master._socket.send(str);
        console.log("Feed. Unregister client", client._id);
        client._feed = undefined;
        IPC.clients.splice(IPC.clients.indexOf[client], 1);
    }
}
/**
 * ----------
