...
function RootComponent(id) {
    ...
    for (let i = 0; i < this._divs.length; i++) {
        $(this._divs[i]).css("position", "absolute");
    }
...
RootComponent.prototype.start = function() {
    ...
    document.body.addEventListener("touchend",function(e) {
        self._eventMouseUp(self._mouseX - self._absoluteX, self._mouseY - self._absoluteY);
        e.preventDefault();
    }, false);
    ...
    $(document).on("tap", function(e) {
        self._eventMouseDown(e.pageX - self._absoluteX, e.pageY - self._absoluteY);
        self._eventMouseUp(e.pageX - self._absoluteX, e.pageY - self._absoluteY);
    });
...
RootComponent.prototype.addWindow = function(child) {
    ...
    this._windows.set(child._id, child);
}
/**
 * @param {Component} child
 */
...
//    delete this._windows.get(child._id);
    this._windows.delete(child._id);
...
RootComponent.prototype.bringToFront = function(window) {
    for (let i = 0; i < this._clientContainer._child.length; i++) {
...
//        if (RootComponent._captureMouse.onMouseMove(x-a[0], y-a[1]))
//            return RootComponent._captureMouse;
//    }
    ...
    for (let cid in RootComponent._mouseIn) {
        if (RootComponent._mouseIn.hasOwnProperty(cid)) {
            let c = RootComponent._mouseIn[cid];
            if (!oldMouseIn.hasOwnProperty(cid))
                c.onMouseEnter(x, y);
        }
    }
    for (let cid in oldMouseIn) {
        if (oldMouseIn.hasOwnProperty(cid)) {
            let c = oldMouseIn[cid];
            if (!RootComponent._mouseIn.hasOwnProperty(cid))
                c.onMouseLeave(x, y);
        }
    }
...
RootComponent.prototype.handleMouseMove = function(x, y) {
    ...
    this._mouseX = xx;
    this._mouseY = yy;
    for (let i = this._clientContainer._child.length - 1; i >= 0; i--) {
        let c = this._clientContainer._child[i];
...
RootComponent.prototype.handleMouseDown = function(x, y, b) {
    ...
    if (xx < 0 || yy < 0 || (xx > this._clientContainer._width) || (yy > this._clientContainer._height)) {
        console.log("RootComponent: onMouseDown out of border");
...
RootComponent.prototype._refresh = function() {
    ...
    for (let i = 0; i < RootComponent._createList.length; i++) {
        let c = RootComponent._createList[i];
 ...
 * @param {number} h
 */
RootComponent.prototype.onWindowSetBounds = function(window, x, y, w, h) {
    var maxw=0;
    var maxh=0;
    var c;
    for(var i = 0; i < this._clientContainer._child.length; i++) {
        c = this._clientContainer._child[i];
        if (c._width + c._x > maxw)
            maxw = c._x + c._width;
        if (c._height + c._y)
            maxh = c._y + c._height;        
    }
    if (maxw !== this._desktopWidth || maxh !== this._desktopHeight) {
        this._desktopWidth = maxw;
        this._desktopHeight = maxh;
    }
    this._updateSize();
}
