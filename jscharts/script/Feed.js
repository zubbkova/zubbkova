/* global FeedContent, IPC, Main, _, StatusLight */
/**
 * ----
 * Feed
 * ----
...
//    console.log("feed process", this._name, this._feedItems.length);
    if (this._feedItems.length === 0) 
        return false;
    var processItems = this._feedItems;
    this._feedItems = [];
...
Feed.prototype.start = function() {
    this._error = -1;
    IPC.register(this);
    IPC.userName = decodeURIComponent(Main.getUserName().replace(/\+/g, '%20'));
    IPC.sid = Main.getSID();
}
/**
 * @param {number} error
...
Feed.prototype.onError = function(error) {
    var message = "Feed for " + this._name + " stopped. Error: ";
    switch (error) {
        case 0:
    ...
    this.stop();
    if (error !== 5) {
        window.location = Main.getAdvfnURL() + '/p.php?pid=message&msg=' + error;
    }
}
/**
 * @param {string=} dataBlock
...
Feed.prototype.stop = function(dataBlock) {
    ...
    var data = dataBlock ? dataBlock : this._dataBlock;
