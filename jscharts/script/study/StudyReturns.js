...
StudyReturns.prototype.updateDefaultDataSource = function() {
    ...
    this._bclose = this._parent._chartCanvas._chart._currentSymbol.size() > 1 ? this._parent._chartCanvas._chart.getSeries(Chart.S_OVERLAY) : undefined;
}
/** @override */
StudyReturns.prototype.setName = function() {
    this._name = Language.getString("study_logreturns");
    if (this._parent._chartCanvas._chart._currentSymbol.size() > 1) {
        this._name += " " + Language.getString("study_relativeto") + " " + this._parent._chartCanvas._chart._currentSymbol.getDisplaySymbol(1);
    }
}
/** @override */
...
StudyReturns.prototype.update = function(start, end) {
    ...
    let lasta = this._close.get(i._d);
    let lastb = this._bclose ? this._bclose.get(i._d) : 1.0;
    this._series.append(i._d, 0.0);
    i.move();
    do {
        let a = this._close.get(i._d);
        let b = this._bclose ? this._bclose.get(i._d) : 1.0;
        let out = 0.0;
        if (a !== 0) {
            if (b !== 0) {
