...
StudyHighLow.prototype.setParams = function(params) {
    ...
    if (items.has("use_closed_price"))
        this._use_closed_price = items.get("use_closed_price").toString() === "true";
...
StudyHighLow.prototype.draw = function() {
    ...
    let end = c._parent._currentSymbol._timeEnd > this._source.timeEnd() ? this._source.timeEnd() : c._parent._currentSymbol._timeEnd;
    let start = c._parent._currentSymbol._timeStart < this._source.timeStart() ? this._source.timeStart() : c._parent._currentSymbol._timeStart;
    let curVal = 0;
    let minCurVal = 0;
    let maxCurVal = 0;
    let curDate; // the most extreme point while tracking (date)
