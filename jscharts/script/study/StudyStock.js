...
function StudyStock(overlay, si) {
    ...
    var info = overlay._chart._currentSymbol.getSymbolInfo(si);
    if (info)
...
StudyStock.prototype.updateDefaultDataSource = function() {
    if (this._symbolIndex < this._parent._chart._currentSymbol.size()) {
        this._open = this._parent._chart._currentSymbol.getSeries(this._symbolIndex, Chart.S_CUR_OPEN);
        this._high = this._parent._chart._currentSymbol.getSeries(this._symbolIndex, Chart.S_CUR_HIGH);
        this._low = this._parent._chart._currentSymbol.getSeries(this._symbolIndex, Chart.S_CUR_LOW);
        this._close = this._source = this._parent._chart._currentSymbol.getSeries(this._symbolIndex, Chart.S_CUR_CLOSE);
    }
}
/** @override */
...
/** @override */
StudyStock.prototype.drawPrice = function() {
    // draw prices
    if (!this._isOverlay) {
        if (this._parent._chart._currentSymbol._timeEnd < this._parent._chart._currentSymbol._time) {
            this._parent.drawPrice(this._close.get(this._parent._chart._currentSymbol._timeEnd), Color.gray);
        }
        this._parent.drawPrice(this._close.get(this._parent._chart._currentSymbol._time), Color.black);
    }
}
/** @override */
StudyStock.prototype.draw = function() {
    ...
    switch (this._style) {
        ...
        case StudyStock.ST_HEIKENASHI:
            this._parent.drawHeikenAshi(this._open, this._high, this._low, this._close, StudyStock.HA_tails, StudyStock.HA_tails, Color.brightGreen, Color.red);
            break;
...
StudyStock.createMainStock = function(overlay) {
    var ss = new StudyStock(overlay, 0);
    ss._colour = (overlay._chart._features & Chart.F_LINE_BLUE) == Chart.F_LINE_BLUE ? Color.blue : Color.black;
    return ss;
}
/** 
 * @static
...
StudyStock.createOverlayStock = function(overlay, o) {
    var ss = new StudyStock(overlay, o + 1);
    ss._name = overlay._chart._currentSymbol.getDisplaySymbol(o + 1);
    ss._initLegend();
    return ss;
}