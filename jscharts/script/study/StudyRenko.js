...
StudyRenko.prototype._drawRenkoBoxes = function(startx, endx, startPrice, endPrice) {
    if ((startx === 0 && endx === 0) || startx === endx)
        return;
    let up = endPrice > startPrice ? true : false;
    if (up) {
        // find number of boxes
        let numBoxes = parseInt((endPrice - startPrice) / this._boxSize, 10);
        if (numBoxes <= 0) 
            return;
        // find width and height of each box
        let boxWidth = (endx - startx) / numBoxes;
        let boxHeight = parseInt(this._parent.getY(0) - this._parent.getY(this._boxSize), 10);	// bit of a hack this, doesn't account for log scale
        let starty = parseInt(this._parent.getY(startPrice), 10);
        // draw
        ...
        // find number of boxes
        let numBoxes = parseInt((startPrice - endPrice) / this._boxSize, 10);
        if (numBoxes <= 0) 
            return;
        // find width and height of each box
        let boxWidth = (endx - startx) / numBoxes;
...
StudyRenko.prototype.draw = function() {
    ...
    let endx;
    let startPrice = this._series.getByIndex(0);
    let endPrice = this._series.getByIndex(1); 
    let currentDirection = endPrice > startPrice ? true : false;
    for (let n = 1; n < this._series.size(); n++) {
        ...
        let myDirection = endPrice > startPrice ? true : false;
...
StudyRenko.prototype.update = function(start, end) {
    ...
    let boundaryLow, boundaryHigh; // the values that would have to be exceeded to cause another box or a reversal. 
    ...
    let close1 = this._close.get(ti._d);
    ti.move();
    let close2 = this._close.get(ti._d);
