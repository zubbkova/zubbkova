...
StudyBOS.prototype.updateDefaultDataSource = function() {
    ...
    this._bid = this._parent._chartCanvas._chart.getSeries(Chart.S_BID_CLOSE);
    this._offer = this._parent._chartCanvas._chart.getSeries(Chart.S_OFFER_CLOSE);
}
/** @override */
...
StudyBOS.prototype.setParams = function(params) {
    ...
    if (items.has("format"))
        this._format = parseInt(items.get("format"), 10);
    ...
    if (!this._isIndex && !this._doNotDisplay) {
        ...
        if (this._format % 3 === StudyBOS.SHADING) {
            ...
            if (this._format > 2) {
                ...
                this._parent.drawPrice(this._mid.get(this._parent._chartCanvas._chart._currentSymbol._timeEnd), this._colour);
            }
            this._parent.drawLineNormal(this._bid, Color.blue);
            this._parent.drawPrice(this._bid.get(this._parent._chartCanvas._chart._currentSymbol._timeEnd), Color.blue);
            this._parent.drawLineNormal(this._offer, Color.red);
            this._parent.drawPrice(this._offer.get(this._parent._chartCanvas._chart._currentSymbol._timeEnd), Color.red);
...
StudyBOS.prototype.getColumnNames = function() {
    let s = this._parent._chartCanvas._chart._currentSymbol.mainSymbol();
