...
StudyVolumeEMA.prototype.draw = function() {
    ...
    do {
        ...
        if (!isNaN(end_y) && !(i._d > (c._currentSymbol._time))) {
            var width = Math.max(parseInt(c._currentSymbol._unitWidth, 10) - 2, 1);
            // draw volume stuff
    ...
    var total = 0.0; // running total for EMA
    var current = 0.0; // working value for EMA
    var smooth = 2.0 / (1 + this._period); // smoothing value of EMA
