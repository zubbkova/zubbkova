/* eslint no-empty: "off" */
/* global Study, Language, Utils, TimeIterator, StudyDialog_StudyEditParameter, StudyDialog_StudyButton, XTIterator, Color */
/**
 * Point and Figure
...
StudyPointAndFigure.prototype.roundDownwardsToBoxSize = function(value) {
    var temp = Math.floor(value / this._boxSize);
...
StudyPointAndFigure.prototype.roundUpwardsToBoxSize = function(value) {
    var temp = Math.ceil(value / this._boxSize);
    return temp * this._boxSize;
}
/** @override */
StudyPointAndFigure.prototype.draw = function() {
    ...
    if (this._boxSize === 0.0 || this._currentFrequency !== this._parent._chart._currentSymbol.mainFrequency()) {
        var newBoxSize = this.getBoxSize();
        this._currentFrequency = this._parent._chart._currentSymbol.mainFrequency();
    ...
    if (this._columns.length === 0)
        return;
    var cur = this._columns[0];
    var startX = this._parent.getXOffScale(cur._startDate);
    var endX;
    for (var n = 0; n < this._columns.length; n++) {
        var nextCol = this._columns[n];
    ...
    endX = this._parent.getXOffScale(this._parent._chart._currentSymbol._time); 
    cur.drawColumn(startX, endX);
}
/** @override */
...
StudyPointAndFigure.prototype.getBoxSize = function() {
    var i = XTIterator.reverseScreenIterator(this._parent._chart);
...
StudyPointAndFigure.prototype.update = function(start, end) {
    ...
    var startHigh = this._high.get(ti._d);
    var startLow = this._low.get(ti._d);
    var firstDate = new Date(ti._d.getTime());
    var currentColumn;
    do {
    ...
    do {
        var nextCol = currentColumn.addValues(ti._d, this._low.get(ti._d), this._high.get(ti._d));
        if (nextCol) {
...
StudyPointAndFigure.prototype.setParams = function(params) {
    ...
    if (items.hasOwnProperty("reversal") && typeof items["reversal"] !== "undefined")
        this._reversal = parseInt(items["reversal"], 10);
...
StudyPointAndFigure_Column.prototype.drawColumn = function(startX, endX) {
    ...
    if (!this._upTrend) {
        ...
        c = this._boxHigh;
        do {
            n = c - this._delegate._boxSize;
            curY = parseInt(this._delegate._parent.getY(c), 10);
            if (isNaN(curY)) curY = 0;
            nextY = parseInt(this._delegate._parent.getY(n), 10);
...
StudyPointAndFigure_Column.prototype.addValues = function(cur, low, high) {
    var newLow = this._delegate.roundUpwardsToBoxSize(low);
    var newHigh = this._delegate.roundDownwardsToBoxSize(high);
    var reversalValue, newCol;
    if (this._upTrend) {
        if (newHigh > this._boxHigh) {
            this._boxHigh = newHigh;
            return;
        }
        reversalValue = this._boxHigh - this._delegate._reversal * this._delegate._boxSize;
        if (newLow <= reversalValue) {
            newCol = new StudyPointAndFigure_Column(new Date(cur.getTime()), newLow, this._boxHigh - this._delegate._boxSize, this._delegate);
...
//            console.log(this._boxLow);
        ...
        reversalValue = this._boxLow + this._delegate._reversal * this._delegate._boxSize;
        if (newHigh >= reversalValue) {
            newCol = new StudyPointAndFigure_Column(new Date(cur.getTime()), this._boxLow + this._delegate._boxSize, newHigh, this._delegate);
