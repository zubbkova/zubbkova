...
StudyCoppock.prototype.setParams = function(params) {
    ...
    if (items.has("rocperiod"))
        this._rocperiod = parseInt(items.get("rocperiod"), 10);
    if (items.has("emaperiod"))
        this._emaperiod = parseInt(items.get("emaperiod"), 10);
...
StudyCoppock.prototype.update = function(start, end) {
    ...
    let matotal = 0.0;
    let mabuffer = new Array(this._maperiod);
    mabuffer.fill(0.0);
    ...
    do {
        if (this._maseries.get(this._parent._chartCanvas._chart.timeAdjust(i._d, -this._rocperiod)) !== 0.0) {
            let previousroc = this._maseries.get(this._parent._chartCanvas._chart.timeAdjust(i._d, -this._rocperiod));
    ...
    let smooth = 2.0 / (1 + this._emaperiod);
    do {
