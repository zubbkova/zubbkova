...
StudyEff.prototype.draw = function() {
    ...
    let yInterval = this._parent._ySpread / StudyEff.COLUMNS;
    let totalVol = 0;
    // Have to calculate here because mid/spread values aren't set when update() is called.
    for (let i = 0; i < StudyEff.COLUMNS; i++) {
        this._count[i] = 0;
        this._volumes[i] = 0;
        this._eff[i] = 0.0;
    }
    ...
    let maxEff = 0.0;
    let aveVol = totalVol / p;
    do {
        if (i._d > this._source.timeByIndex(-1))
            continue;
        let closeVal = this._source.get(i._d);
        let highVal = this._high.get(i._d);
        let lowVal = this._low.get(i._d);
        let move = Math.abs(highVal - lowVal) / ((highVal + lowVal) / 2.0); // percentage move
        let pvol = this._vol.get(i._d);
        let hidx = parseInt((highVal - yStart) * 50.0 / this._parent._ySpread, 10);
        let lidx = parseInt((lowVal - yStart) * 50.0 / this._parent._ySpread, 10);
    ...
    for (let t = 0; t < StudyEff.COLUMNS; t++) {
        if (this._count[t] === 0) {
            continue;
        }
    ...
    for (let t = 0; t < StudyEff.COLUMNS; t++) {
        ...
        let curyb = parseInt(this._parent.getY(labely), 10);
        let curye = parseInt(this._parent.getY(labely + yInterval), 10);
        let width = parseInt(75 * this._eff[t] / maxEff, 10);
        this._parent._chartCanvas.fillRectWithAdjust(this._parent._chartCanvas._chart._drawX + 1, curyb + 1, width, (curyb - curye) - 1);
    }
}
/** @override */
