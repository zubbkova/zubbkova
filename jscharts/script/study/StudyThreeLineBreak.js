...
StudyThreeLineBreak.prototype.setParams = function(params) {
    ...
    if (items.hasOwnProperty("lines") && typeof items["lines"] !== "undefined")
        this._lines = parseInt(items["lines"], 10);
...
StudyThreeLineBreak.prototype.update = function(start, end) {
    if (!start || !end)
        return;
    this._series.clear();
    var direction = true; // true is up, false is down.
    var lineCount = 1; // the number of lines we've been going in the same direction
    // get the first item
    var i = TimeIterator.forwardRangeIterator(this._parent._chart.getMasterTimeList(), start, end);
    var lastHigh = this._source.get(start); // the values we are comparing against
    var lastLow = this._source.get(start); // if we're in a break, then they are the high and low of the range
    ...
    if (this._source.get(i._d) >= lastHigh)
        ...
        direction = false;
    do {
        var value = this._source.get(i._d);
...
StudyThreeLineBreak.prototype.draw = function() {
    ...
    var colourUp = Color.brightGreen;
    var colourDown = Color.red;
    this.updateDefaultDataSource();
    Study.prototype.update.call(this);
    var startx = this._parent.getXOffScale(this._series.timeByIndex(0));
    var endx;
    var starty = this._parent.getY(this._series.getByIndex(0));
    var endy;
    this.updateY();
    for (var n = 1; n < this._series.size(); n++) {
        ...
        if (startx < (c._drawX + c.getDrawGraphWidth()) || endx < (c._drawX + c.getDrawGraphWidth())) {
            if (endy < starty) {
                ...
                this._parent._chartCanvas.fillRectWithAdjust(startx, parseInt(endy, 10), endx - startx, parseInt(starty - endy, 10));
            } else {
                this._parent._chartCanvas.setFillColor(colourDown);
                this._parent._chartCanvas.fillRectWithAdjust(startx, parseInt(starty, 10), endx - startx, parseInt(endy-starty, 10));
            }
        }
        startx = endx;
        starty = endy;
