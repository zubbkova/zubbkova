...
/* global MaxMin, Series, Chart, Legend, DecimalFormat, Language */
/**
 * -----
 * Study
 * -----
...
function Study(overlay) {
    ...
    this._range = new MaxMin(overlay._chart);
    this._comboMnemonics = Study.getMnemonics();
    this._series = new Series();
    this.updateDefaultDataSource();
}
Study.prototype.updateDefaultDataSource = function() {
    this._open = this._parent._chart.getSeries(Chart.S_CUR_OPEN);
    this._high = this._parent._chart.getSeries(Chart.S_CUR_HIGH);
    this._low = this._parent._chart.getSeries(Chart.S_CUR_LOW);

    if (typeof this._close === "undefined" || this._close === this._source)
        this._close = this._source = this._parent._chart.getSeries(Chart.S_CUR_CLOSE);
    else
        this._close = this._parent._chart.getSeries(Chart.S_CUR_CLOSE);

    this._buy = this._parent._chart.getSeries(Chart.S_BUY_VOLUME);
    this._sell = this._parent._chart.getSeries(Chart.S_SELL_VOLUME);
    this._unknown = this._parent._chart.getSeries(Chart.S_UNKNOWN_VOLUME);
    this._vol = this._parent._chart.getSeries(Chart.S_TOTAL_VOLUME);
    if (this._source) {
        if (this._source.timeStart().getTime() > 0 && this._source.timeStart() < this._parent._chart._currentSymbol._time) {
            this.update(this._source.timeStart(), this._parent._chart._currentSymbol._time);
        }
    }
}
/**
...
Study.prototype.drawPrice = function() {}
...
Study.prototype.checkRange = function(s) {
    ...
    var ss = this._parent._chart._currentSymbol;
    var size = ss.getMasterTimeList().count(s.timeStart(), s.timeEnd());
    if (range > size) {
        ss.setStart(this._parent._chart.timeAdjust(ss.getStart(), -range));
        return false;
    }
    return true;
}
...
Study.prototype.setParams = function(p) {
    ...
    this._parent._chart._currentSymbol.setStart(new Date(this._parent._chart._currentSymbol.getMasterTimeList().add(this._parent._chart._currentSymbol._timeStart, -this._parent._chart.calcMaxStudyRange())));
}
...
Study.prototype.getMnemonic = function() {
    return (typeof this.constructor.mnemonic === "undefined") ? "" : this.constructor.mnemonic;
}
...
Study.studyMnemonicToIndex = function(m) {
    for (var i = 0; i < Study.comboMnemonics.length; i++) {
...
Study.getMnemonics = function() {
    var temp = [[Language.getString("study_nostudy"), ""] ,
