/* global Study, Language, Color, XTIterator, Chart */
/**
 * @constructor
 * @extends {Study}
 * @param {Overlay} o
...
StudyVolPlus.prototype.draw = function() {
    ...
    if (typeof this._vol === "undefined")
        this.updateDefaultDataSource();
    var lastPrice = 0, currentPrice = 0; // for calculating prices
    var cx = parseInt(c._currentSymbol._unitWidth / 2, 10);
    var i = XTIterator.forwardScreenIterator(c);
    var start_y = parseInt(this._parent.getY(0), 10);
    do {
        if (this._vol.get(i._d) === 0.0)
            continue;
        var end_y = this._parent.getY(this._vol.get(i._d));
