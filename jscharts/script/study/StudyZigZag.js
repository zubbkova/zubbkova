/* global Study, Series, Language, Utils, TimeIterator, Color, StudyDialog_StudyEditParameter */
/**
 * @constructor
 * @extends {Study}
 * @param {Overlay} o
...
StudyZigZag.prototype.setParams = function(params) {
    ...
    if (items.hasOwnProperty("percentage") && typeof items["percentage"] !== "undefined")
        this._percentage = parseInt(items["percentage"], 10);
...
StudyZigZag.prototype.draw = function() {
    ...
    var start = c._parent._currentSymbol._timeStart < this._source.timeStart() ?
            this._source.timeStart() : c._parent._currentSymbol._timeStart;
    var curVal = 0; // current value we're working with
    var triggerUp, triggerDown; // values that will trigger a line being drawn
    var delta = 0; // +1 / -1, going up or down.  0 for unset.
    var tracker; // are we looking for a trigger or the end of one?
    var curExtreme; // the most extreme point while tracking (value)
    var curExtremeDate; // the most extreme point while tracking (date)
