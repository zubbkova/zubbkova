...
StudyVWAPFeed.prototype._initialiseFeed = function() {
    ...
    fr.cmdSymbol(this._parent._parent._chartCanvas._chart._parent._currentSymbol.mainSymbol());
...
StudyVWAPFeed.prototype.feedDelegate_feed = function(contents) {
    if (contents._id === 0) { // time
        ...
        if (this._undelayedTime) {
            let val = this._parent._close.get(this._undelayedTime);
            this._undelayedTime = this._parent._parent._chartCanvas._chart._parent._symbolSets.get(0).getMasterTimeList().convertTimestamp(ts, PriceDataConstants.FREQUENCY_1);
            if(!isNaN(val)) {
                let setTime = new Date(this._undelayedTime.getTime()); // we're ignoring delay for now, that may not be wise.  but it's one hell of a slog to get it.
                ...
                this._parent._parent._chartCanvas._chart.repaint();
            }
        } else {
            this._undelayedTime = this._parent._parent._chartCanvas._chart._parent._symbolSets.get(0).getMasterTimeList().convertTimestamp(ts, PriceDataConstants.FREQUENCY_1);
        }
