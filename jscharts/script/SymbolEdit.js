...
/* global Component, Language, Label, EditBox, ComboBox, Color, SymbolRequest, ChartEvent, Utils */
/**
 * ----------
 * SymbolEdit
 * ----------
...
SymbolEdit.prototype.setSymbol = function(s) {
    ...
    if (Utils.getConstructorName(s) === "ChartSymbol") {
...
//    } else
        ...
        var i = this._multiCombo.getIndex();
...
SymbolEdit.prototype.updateSymbol = function(f, w) {
    var force = f === undefined ? false : f;
    var withEvent = w === undefined ? false : w;
    var text = this._editBox.getText();
    if (text.length === 0) {
        // overlays - just clean
        if (!this._isMain) {
            this._curSymbol = undefined;
