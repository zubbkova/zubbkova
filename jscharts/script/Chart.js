...
Chart.prototype.setStudy = function(slot, mnemonic) {
    ...
    if (StudyFactory.studyHasOverlay(mnemonic)) {
        ...
        for (let s of this._canvas._overlays) {
            if (s && s._legend)
...
Chart.prototype.setBounds = function(x, y, width, height) {
    ...
    let deltaH = height - this._height;
    if (this._currentSymbol) {
    ...
    if (this._mainOverlay) {
        if (!this._canvas)
            return;
        let mainHeight = this._drawHeight;
...
Chart.prototype.getSeries = function(index) {
    ...
    let si = index - Chart.S_OVERLAY + 1;
...
Chart.prototype.initialiseParams = function() {
    ...
    this._tooltips.push(Language.getString("tooltip_click_to_add"));
    this._localCalendar = Calendar.getInstance("UTC");
    if (Main.getParams().has("drawingThickness")) {
        let t = parseInt(Main.getParams().get('drawingThickness').toString(), 10);
        this._drawingThickness = t;
        if (this._parent._miscBar) {
            this._parent._miscBar.resetThickness(t - 1);
        }
    }
    if (Main.getParams().has("objectColour")) {
        let colourParam = Main.getParams().get("objectColour");
...
Chart.prototype.initialiseDrawArea = function() {
    ...
    if (Main.getParams().has("f")) {
        this._features = parseInt(Main.getParams().get("f").toString(), 10);
    } else {
        this._features = 0;
    }
    ...
    if (Main.getParams().has("yc")) {
        this.setShowYestClose(Main.getParams().get("yc").toString() === "1");
    } else {
...
Chart.prototype.initialiseStudies = function() {
    for (let i = 0; i < Chart.MAX_STUDIES; i++) {
        ...
        if (Main.getParams().has("s" + i)) {
            this.setStudy(i, Main.getParams().get("s" + i));
            if (Main.getParams().has("sp" + i)) {
                this.setStudyParams(i, Main.getParams().get("sp" + i));
            }
        }
    }
    this._recalcStudies = false;
...
Chart.prototype.allStudies = function() {
    ...
    for (let i = 0; i <= Chart.MAX_STUDIES; i++) {
        if (this._canvas._overlays[i]) {
            for (let j = 0; j < Chart.MAX_STUDIES; j++) {
...
Chart.prototype.calcMaxStudyRange = function() {
    let maxRange = 0;
    for (let i = 0; i < this._studylist.length; i++) {
        let s = this.getStudy(i);
        if (s) {
...
Chart.prototype.resizeOverlays = function() {
    ...
    newOverlayH = Math.trunc(newOverlayH);
    let curY = this._drawY + this._drawHeight - oCount * newOverlayH;
    ...
    for (let i = 1; i <= Chart.MAX_STUDIES; i++) {
        ...
        if (this._canvas._overlays[i]._legend) {
            this._canvas._overlays[i]._legend.resetLocation();
        }
        curY += newOverlayH;
    }
}
        ...
        for (let i = 0; i <= Chart.MAX_STUDIES; i++) {
            if (this._canvas._overlays[i] && this._canvas._overlays[i]._autoScale) {
                ...
                if (this._canvas._overlays[i] && this._canvas._overlays[i]._study) {
                    for (let j = 0; j < this._canvas._overlays[i]._study.length; j++) {
                        if (this._canvas._overlays[i]._study[j]) {
                            let it = XTIterator.reverseScreenIterator(this);
...
Chart.prototype._setDrawAreaSize = function() {
    ...
    let copyrightHeight = ((this._features & Chart.F_NO_COPYRIGHT) === Chart.F_NO_COPYRIGHT) ? 0 : this._fontSize + 3;
    ...
    this._xSFWidth = Math.trunc((this._drawWidth / 4.0));
...
Chart.prototype._createOverlay = function(slot, mnemonic) {
    if (!this._canvas)
        return;
    let o = new Overlay(this._canvas, 0, 0);
...
Chart.prototype._checkObjectSelection = function(x, y) {
    if (this._currentSymbol === undefined) 
        return -1;
    for (let i = 0; i < this._currentSymbol._drawObjects.length; i++) {
        let cur = this._currentSymbol._drawObjects[i];
        let selPoint = cur.hasGotSelectionPoint(new Point(x, y));
        if (selPoint !== -1) {
...
Chart.prototype._addDrawingObject = function() {
    if (this._currentSymbol._drawObjects.length === Chart.MAX_DRAW_OBJECTS) 
        return;
    let newObj = this._currentDrawObject.copy();
...
Chart.prototype._adjustOverlays = function(slot, offset) {
    ...
    if (this._canvas._overlays[i] === undefined) 
        return;
    let newH1 = this._currentSymbol._savedWindowSizes[0][1] - offset;
    let newH2 = this._currentSymbol._savedWindowSizes[1][1] + offset;
...
Chart.prototype._buildParams = function(s1, s2) {
    ...
    let h = this._parent.buildParams(true);
    h.delete("period");
    h.set("from", (this._currentSymbol._timeStart.getTime() / 60000).toString());
    h.set("to", (this._currentSymbol._timeEnd.getTime() / 60000).toString());
    h.set("ls", this._currentSymbol._currentLabelScale.toString());
    let h2 = this._currentSymbol.getDrawingObjects();
    Utils.mergeHashtables(h, h2, true);
...
Chart.prototype.onMouseDown = function(x, y, b) {
    this._b = b;
    if (this._capture) {
        let res = this._capture.onMouseDown(x, y);
        return res;
    }
    ...
    if (this._mode !== Chart.MODE_NORMAL) {
        if (!this._drawingNow) {
            ...
            this._currentDrawObject._colour = this._objectColour;
    ...
    let i = this._checkObjectSelection(x, y);
    if (i !== this._objectSelected) {
        ...
        if (this._objectSelected !== -1) {
            let curObj = this._currentSymbol._drawObjects[this._objectSelected];
...
Chart.prototype.onMouseUp = function(x, y) {
    ...
    let timeSinceLastClick = curClickTime - this._timeOfLastClick;
...
Chart.prototype.onMouseMove = function(x, y) {
    ...
    for (let i = 0; i <= Chart.MAX_STUDIES; i++) {
        if (this._canvas._overlays[i] === undefined) 
            continue;
        let bottomY = this._canvas._overlays[i]._y + this._canvas._overlays[i]._height;
        if (y < bottomY - 1) {
            this._mouseArea = i;
            break;
    ...
    for (let i = 0; i <= Chart.MAX_STUDIES; i++) {
        if (this._canvas._overlays[i] && this._canvas._overlays[i]._legend) {
            let isOpen = this._mouseArea === i && this._canvas._overlays[i]._showLegend;
    ...
    if (this._mouseAreaType === Chart.M_IN_WINDOW) {
        if (this._mode !== Chart.MODE_NORMAL && this._drawingNow) {
            if (!this._currentDrawObject._drawAcrossWindows || this._mouseArea === this._lineStartArea) {
                this._currentDrawObject.onMouseMove(x, y);
                this.repaint();
                this.process();
            }
...
Chart.prototype.onMouseDrag = function(x, y) {
    ...
    if (this._mode === Chart.MODE_NORMAL) {
        if (this._b === 3) {
            ...
            if (this._mouseAreaType === Chart.M_IN_X_AXIS) {
                this.setCursor(Cursor.COL_RESIZE_CURSOR);
                let pow = Math.min(2.0, (this._dragX - x) / this._xSFWidth);
                    ...
                    if (this._objectSelected !== -1) {
                        ...
                        if (this._dragX < (this._drawX + this._drawWidth)) {
                            ...
                            if (Math.abs(dx) >= 1.0) {
                                ...
                                this._currentSymbol.setTimeEnd(this.timeAdjust(this._currentSymbol._oldTimeEnd, Math.trunc(-dt)));
                            }
                        }
                        this._canvas._overlays[this._mouseArea].onMouseDrag(x, y);
                    }
...
Chart.prototype.onKeyUp = function(keyCode) {
    ...
    switch (keyChar) {
        ...
        case '2':
            ...
            break;
        case 'f':
            let sr = this._canvas._overlays[0]._study[0]._series;
            let msg = sr.size() + " items" + eol;
            for (let j = 0; j < sr.size(); j++) {
                msg += sr.timeByIndex(j) + " - " + Math.trunc(sr.timeByIndex(j).getTime() / 1000) + " - " + sr.getByIndex(j) + eol;
            }
            msg += eol;
            break;
        case 's':
        ...
        case 'x':
            ...
            let i = XTIterator.reverseScreenIterator(this);
            do {
                msg += "x = " + i._x + " d = " + i._d + " t = " + Math.trunc(i._d.getTime() / 1000);
...
//            msg = "";
//            for (let key in em) {
//                msg += key + "=" + Main.getParams().get(key) + eol;
//            }
            break;
        case 'i':
            break;
...
Chart.prototype.onCustomEvent = function(e) {
    if (this.isValid() && e._event === ChartEvent.MODAL_OK) {
        if (e._source.constructor.name === 'CalloutDialog') {
            let curObj = this._currentSymbol._drawObjects[this._objectSelected];
            curObj.setText(e._source.getText());
        } else if (e._source.constructor.name === 'ParamDialog') {
            let curObj = this._currentSymbol._drawObjects[this._objectSelected];
