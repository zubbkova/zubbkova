...
function ParamDialog(id, delegate, l, v) {
    ...
    for (let i = 0; i < this._numParams; i++) {
        let l1 = new Label(this._id + "_l" + i, this);
...
ParamDialog.prototype.getParams = function() {
    let p = new Array(this._numParams);
    for (let i = 0; i < this._numParams; i++)
...
ParamDialog.prototype.setModal = function(open) {
    if (Main.getSession().getRootComponent()._modal && Main.getSession().getRootComponent()._modal.constructor.name === "ParamDialog" && open)
        return;
    if (open !== this._shown) {
