...
List.prototype.draw = function(force) {
    ...
    if (this._drawTable) {
        ...
        $("#" + tid + " tr").bind("touchstart", function(e) {
            self._curY = e.originalEvent.targetTouches[0].pageY;
        });
        $("#" + tid + " tr").bind("touchend", function(e) {
            if (self._curY == e.originalEvent.changedTouches[0].pageY) {
                self._curRow = $(this).attr("rownum");
                self.onClick(self._curRow);
            }
        });
...
List.prototype.onMouseUp = function(x, y) {
    if (typeof this._curRow !== 'undefined')
