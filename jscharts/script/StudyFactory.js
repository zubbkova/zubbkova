...
var StudyFactory = {};
StudyFactory.studies = new Map();
for (let i = 0; i < Study.studyClasses.length; i++) {
    var c = Study.studyClasses[i];
    if (c.hasOwnProperty("mnemonic")) {
        StudyFactory.studies.set(c.mnemonic, c);
    }
}
/**
 * @param {Overlay} ov
...
StudyFactory.getStudy = function(ov, mnemonic) {
    let c = StudyFactory.studies.get(mnemonic);
    if (c === undefined) {
        console.error("Undefined study:", mnemonic);
        return;
    }
    let moo = c.newInstance(ov);
    moo.setName();
    moo._initLegend();
    return moo;
}
...
StudyFactory.getHelpCode = function(mnemonic) {
    var c = StudyFactory.studies.get(mnemonic);
    if (!c) return;
    return c.helpID;
}
/**
